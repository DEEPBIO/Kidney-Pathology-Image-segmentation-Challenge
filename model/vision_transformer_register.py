...
# https://github.com/lucidrains/soft-moe-pytorch/tree/main
import math
...
class Encoder(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        torch._assert(input.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")
        N1 = input.size(1) 
        N2 = self.pos_embedding.size(1)
        if N1 > N2 : 
            register_tokens, main_tokens =  torch.split(input, [N1-N2, N2], dim=1)
            main_tokens = main_tokens + self.pos_embedding
            input = torch.cat([register_tokens, main_tokens], dim=1)
        elif N1 == N2 : 
            input = input + self.pos_embedding
        else : 
            raise Exception(f'Inpuy shape:{input.shape}, Pos_embedding_shape: {self.pos_embedding.shape}')
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        seq_length += 1
        self.register_token = nn.Parameter(torch.randn(1, 4, hidden_dim))
        #seq_length += 4
